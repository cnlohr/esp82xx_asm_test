...
#define GetCycleCount() ({ uint32_t ccount; __asm__ __volatile__("rsr %0,ccount":"=a"(ccount)); ccount; })
#define NOP() {__asm__ __volatile__("nop");}

void  __attribute__((noinline)) RunTest()
{
	uint32_t baseline = 0;
	os_printf( "NOP: %d\n", baseline = ASMNop( baseline ) );
	os_printf( "ASMTest1: %d\n", ASMTest1( baseline ) );
	os_printf( "ASMTest2: %d\n", ASMTest2( baseline ) );
	os_printf( "ASMTest3: %d\n", ASMTest3( baseline ) );
}
